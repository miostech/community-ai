...
export default function MarcaCriadoresPage() {
    const theme = useTheme();
    const { account } = useAccount();
    const showPdfDownload = account?.role === 'moderator' || account?.role === 'admin';
    ...
    useEffect(() => {
        setLoading(true);
        fetch(`/api/marca/criadores?sortBy=${sortBy}`)
            .then((r) => r.json())
            .then((data) => {
    ...
    const handleDownloadPdf = async () => {
        ...
        try {
            ...
            const fileName = `dome-creators-portal-marca-${new Date().toISOString().slice(0, 10)}.pdf`;
            pdf.save(fileName);
        } catch (e) {
            console.error('PDF:', e);
        } finally {
                ...
                <>
                    ...
                    >
                        <Stack
                            ...
                            justifyContent={showPdfDownload ? 'space-between' : 'flex-start'}
                            spacing={2}
                            sx={{ mb: 3 }}
                        >
                            <Typography
                            ...
                            {showPdfDownload && (
                                <Box data-html2canvas-ignore sx={{ alignSelf: { xs: 'flex-start', sm: 'center' } }}>
                                    <Button
                                        variant="outlined"
                                        size="small"
                        ...
                        <Typography variant="body2" color="text.secondary" sx={{ mb: 3 }}>
                            Top creators da comunidade Dome e visão agregada da base — o mesmo portfólio usado na apresentação interna.
                        </Typography>
