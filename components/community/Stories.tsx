    ...
    >
      <div className="flex gap-4 px-4 py-3" style={{ width: 'max-content' }}>
        {users.map((user, index) => (
          <div 
            key={user.id} 
            className="flex flex-col items-center flex-shrink-0 cursor-pointer group animate-fade-in-up"
            style={{ animationDelay: `${index * 0.05}s` }}
            onTouchStart={() => setPressedStory(user.id)}
            onTouchEnd={() => setPressedStory(null)}
            onMouseDown={() => setPressedStory(user.id)}
